...
class Buttons(nextcord.ui.View):
  ...
  async def rock(self,button:nextcord.ui.Button,interaction:nextcord.Interaction):
    ...
    if check_win("rock",user) == "lose":
      ...
      embed.set_author(name="Dank Memer",icon_url="https://cdn.discordapp.com/avatars/270904126974590976/d60c6bd5971f06776ba96497117f7f58.png?size=64")
      b1=self.children
      for item in b1:
        item.disabled = True
...
class Game(commands.Cog):
  ...
  async def snake(self, ctx):
      ...
      s = SnakeGame(board)
      embed = nextcord.Embed(title='Snake', description=self.format_board(s.get_board(), s.get_head_position()), color=nextcord.Color.blurple())
      msg = await ctx.send(embed=embed)
      emojis = ['⬅','⬆','➡','⬇']
      for emoji in emojis:
          await msg.add_reaction(emoji)
      while True:
          await msg.edit(embed=nextcord.Embed(title='Snake', description=self.format_board(s.get_board(), s.get_head_position()), color=nextcord.Color.blurple()))
          try:
              reaction, _ = await self.client.wait_for('reaction_add', check = lambda r, u: u == ctx.author and str(r) in emojis and r.message == msg, timeout=1.5)
